...
from animatediff.models.epi_module import get_epi_module
...
class UNetMidBlock3DCrossAttn(nn.Module):
    def __init__(
        ...
        self.epi_modules = nn.ModuleList(epi_modules) if use_epi_module else epi_modules
    ...
    def forward(self, hidden_states, F_mats=None, H_mats=None, temb=None, encoder_hidden_states=None, attention_mask=None,
        ...
        sync_lora_scale = getattr(self, "sync_lora_scale", None)
        if sync_lora_scale != None:
            cross_attention_kwargs.update({"sync_lora_scale": sync_lora_scale})
            motion_cross_attention_kwargs.update({"sync_lora_scale": sync_lora_scale})

        additional_outputs = []
        for attn, resnet, motion_module, epi_module in zip(self.attentions, self.resnets[1:], self.motion_modules, self.epi_modules):
            hidden_states = rearrange(hidden_states, "b c f h w -> (b f) c h w")
            ...
            if epi_module is not None:
                hidden_states, aux = epi_module(hidden_states, F_mats, H_mats, temb=temb, encoder_hidden_states=encoder_hidden_states, cross_attention_kwargs=motion_cross_attention_kwargs)
        ...
        return hidden_states, additional_outputs
...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, F_mats=None, H_mats=None, temb=None, encoder_hidden_states=None, attention_mask=None,
        ...
        additional_outputs = []
        for resnet, attn, motion_module, epi_module in zip(self.resnets, self.attentions, self.motion_modules, self.epi_modules):
        ...
        return hidden_states, output_states, additional_outputs 
...
class DownBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, F_mats=None, H_mats=None, temb=None, encoder_hidden_states=None, motion_module_alpha=1.,
        ...
        additional_outputs = []
        for resnet, motion_module, epi_module in zip(self.resnets, self.motion_modules, self.epi_modules):
            if self.training and self.gradient_checkpointing:
                raise NotImplementedError
...
class UpBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, F_mats=None, H_mats=None, temb=None, upsample_size=None, encoder_hidden_states=None,
