...
from animatediff.models.attention_processor import LoRAAttnProcessor as CustomizedLoRAAttnProcessor
from animatediff.models.attention_processor import AttnProcessor as CustomizedAttnProcessor
from animatediff.models.resnet import (
    InflatedConv3d,
    FusionBlock2D
)
...
class UNet3DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
            ...
            # whether output additional channel for depth/other losses
            additional_channel: int = 0,
    ):
        super().__init__()
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            down_block = get_down_block(
                ...
                use_epi_module=use_epi_module and (res in epi_module_resolutions),
                epi_module_kwargs=epi_module_kwargs,
        ...
        if mid_block_type == "UNetMidBlock3DCrossAttn":
            self.mid_block = UNetMidBlock3DCrossAttn(
                ...
                use_epi_module=use_epi_module and epi_module_mid_block,
                epi_module_kwargs=epi_module_kwargs,
            )
        else:
        ...
        if additional_channel > 0:
            self.conv_auxiliary_query = InflatedConv3d(block_out_channels[0], additional_channel, kernel_size=1)
            self.conv_auxiliary_key = InflatedConv3d(block_out_channels[0], additional_channel, kernel_size=1)
    ...
    @property
    def epi_attn_processors(self) -> Dict[str, AttentionProcessor]:
        r"""
        Returns:
            `dict` of attention processors: A dictionary containing all attention processors used in the model with
            indexed by its weight name.
    ...
    def set_epi_attn_processor(self, processor: Union[AttentionProcessor, Dict[str, AttentionProcessor]]):
        r"""
        Sets the attention processor to use to compute attention.
        ...
        """
        count = len(self.epi_attn_processors.keys())
...
class UNet3DConditionModelPoseCond(UNet3DConditionModel):
    ...
    def set_all_attn_processor(self,
        ...
        sync_lora_rank = sync_lora_kwargs.pop('sync_lora_rank')
        sync_lora_scale = sync_lora_kwargs.pop('sync_lora_scale')
        spatial_attn_procs = {}
        if add_spatial:
            ...
            for name in self.attn_processors.keys():
                ...
                if add_pose_adaptor and add_spatial_lora:
                    ...
                    spatial_attn_procs[name] = CustomizedAttnProcessor(attn_type="spatial", spatial_extended_attention=spatial_extended_attention)
            ...
            for name in self.mm_attn_processors.keys():
                ...
                if add_pose_adaptor and add_motion_lora:
                    mm_attn_procs[name] = LORAPoseAdaptorAttnProcessor(hidden_size=hidden_size,
                                                                       ...
                                                                       sync_lora_rank=sync_lora_rank if sync_lora_rank > 16 else hidden_size // lora_rank,
                                                                       sync_lora_scale=sync_lora_scale,
                                                                       **attention_processor_kwargs,
                                                                       **motion_lora_kwargs)
                elif add_pose_adaptor:
                    ...
                    mm_attn_procs[name] = CustomizedAttnProcessor(attn_type="temporal")
            for name in self.epi_attn_processors.keys():
                attention_name = name.split('.')[-2]
                if name.startswith("mid_block"):
                    hidden_size = self.config.block_out_channels[-1]
                    block_id = -1
                    ...
                    block_id = int(name[len("down_blocks.")])
                    hidden_size = self.config.block_out_channels[block_id]
                epi_attn_procs[name] = EpiAttnProcessor(hidden_size=hidden_size) # , **attention_processor_kwargs)
        ...
        self.set_epi_attn_processor(epi_attn_procs)
    ...
    def forward(
        ...
        if pose_embedding_features is None:
            pose_embedding_features = [None for _ in range(len(self.down_blocks))]
        ...
        for downsample_block, pose_embedding_feature, down_fuser, motion_module_alpha in zip(self.down_blocks,
            ...
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples, aux = downsample_block(
                    hidden_states=sample,
                    F_mats=F_mats,
                    H_mats=H_mats,
                    temb=emb,
        ...
        if self.additional_channel > 0:
            ...
            query_list = [x['query'] for x in additional_outputs]
            key_list = [x['key'] for x in additional_outputs]
            last_query = query_list[-1]
            last_key = key_list[-1]
            feat_size = int(last_query.shape[1]**0.5)
            last_query = rearrange(last_query, "(b f) (h w) c -> b c f h w", f=video_length, h=feat_size, w=feat_size)
            last_key = rearrange(last_key, "(b f) (h w) c -> b c f h w", f=video_length, h=feat_size, w=feat_size)

            # for x in additional_outputs:
            #     print(x['query'].shape)
            auxiliary_query = self.conv_auxiliary_query(last_query)
            auxiliary_key = self.conv_auxiliary_key(last_key)
            auxiliary = torch.cat([auxiliary_query, auxiliary_key], dim=1)
        else:
            auxiliary = None

        if (not return_dict):
            return (sample_out, auxiliary)
        elif debug:
            return UNet3DConditionOutput(sample=sample_out, auxiliary=auxiliary), activations
        else:
            return UNet3DConditionOutput(sample=sample_out, auxiliary=auxiliary)
