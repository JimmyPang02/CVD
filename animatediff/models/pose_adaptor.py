...
class PoseAdaptor2D(nn.Module):
    def __init__(self, unet, pose_encoder):
        super(PoseAdaptor2D, self).__init__()
        self.unet = unet
        self.pose_encoder = pose_encoder

    def forward(self, noisy_latents, timesteps, encoder_hidden_states, pose_embedding, F_mats=None, H_mats=None):
        if pose_embedding is not None:
            assert pose_embedding.ndim == 5
            bs = pose_embedding.shape[0]
            pose_embedding = rearrange(pose_embedding, "b c f h w -> (b f) c h w")
...
class CameraPoseEncoder2D(nn.Module):
    ...
    def __init__(self,
        ...
        super(CameraPoseEncoder2D, self).__init__()
        ...
        for i in range(len(channels)):
            ...
            for j in range(nums_rb):
                if j == 0 and i != 0:
                    down_layers.append(ResnetBlock(channels[i - 1], int(channels[i] / compression_factor),
                                                   down=True, ksize=ksize, sk=sk, use_conv=use_conv))
                elif j == 0:
                    down_layers.append(ResnetBlock(channels[0], int(channels[i] / compression_factor),
                                                   down=False, ksize=ksize, sk=sk, use_conv=use_conv))
                elif j == nums_rb - 1:
                    down_layers.append(ResnetBlock(int(channels[i] / compression_factor), channels[i],
                                                   down=False, ksize=ksize, sk=sk, use_conv=use_conv))
                else:
                    down_layers.append(ResnetBlock(int(channels[i] / compression_factor),
                                                   int(channels[i] / compression_factor),
                                                   down=False, ksize=ksize, sk=sk, use_conv=use_conv))
            self.encoder_down_blocks.append(nn.Sequential(*down_layers))
    ...
    def forward(self, x):
        ...
        # extract features
        features = []
        x = self.encoder_conv_in(x)           # bf c h w

        for down_block in self.encoder_down_blocks:
            x = down_block(x)
            features.append(x)
